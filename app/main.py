...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:3000",
        "http://127.0.0.1:3000",
        "https://ecommerce-frontend-seven-jet.vercel.app"
    ],
...
def setup_production_users(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not existing_admin:
            admin_password = "admin"[:72]  # Truncate to 72 bytes for bcrypt
            admin = models.User(
                username="admin",
        ...
        if not existing_vendor:
            vendor_password = "vendor"[:72]  # Truncate to 72 bytes for bcrypt
            vendor_user = models.User(
                username="vendor1",
                email="vendor1@example.com",
                hashed_password=pwd_context.hash(vendor_password),
                role="VENDOR",
