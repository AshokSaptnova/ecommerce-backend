...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE products DROP COLUMN IF EXISTS dosage;")
    op.execute("ALTER TABLE products DROP COLUMN IF EXISTS product_id;")
    op.execute("ALTER TABLE products DROP COLUMN IF EXISTS tagline;")
    ...
    op.execute("ALTER TABLE products DROP COLUMN IF EXISTS old_mrp;")
    op.execute("ALTER TABLE products DROP COLUMN IF EXISTS mrp;")
    ...
    op.drop_index(op.f('idx_session_cart_session_id'), table_name='session_cart', if_exists=True)
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.execute("CREATE INDEX IF NOT EXISTS idx_session_cart_session_id ON session_cart (session_id);")
    ...
    op.drop_constraint(None, 'products', type_='foreignkey')
    op.drop_index(op.f('ix_products_slug'), table_name='products', if_exists=True)
    op.drop_index(op.f('ix_products_sku'), table_name='products', if_exists=True)
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS ix_products_product_id ON products (product_id);")
    op.alter_column('products', 'updated_at',
    ...
    op.alter_column('products', 'vendor_id',
               existing_type=sa.INTEGER(),
               nullable=True)
    op.drop_index(op.f('ix_orders_session_id'), table_name='orders', if_exists=True)
    op.execute("CREATE INDEX IF NOT EXISTS idx_orders_session_id ON orders (session_id);")
    op.execute("CREATE INDEX IF NOT EXISTS idx_orders_razorpay_payment_id ON orders (razorpay_payment_id);")
    op.execute("CREATE INDEX IF NOT EXISTS idx_orders_razorpay_order_id ON orders (razorpay_order_id);")
    # ### end Alembic commands ###
